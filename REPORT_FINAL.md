...
**작성일** 2026년 6월 10일
**자료 출처** 중앙선거관리위원회 선거통계시스템 [info.nec.go.kr](https://info.nec.go.kr/) · 국가선거정보 개방포털 [data.nec.go.kr](http://data.nec.go.kr/) · 대만 중앙선거위원회 [db.cec.gov.tw](https://db.cec.gov.tw/) · 프랑스 내무부 [data.gouv.fr](https://www.data.gouv.fr/)
...
그리고 이 글 전체는 다음 **가설**을 데이터로 검증하는 과정이기도 합니다.

> **가설:** 동일 득표쌍은 조작의 흔적이 아니라, **① 투표소가 많을수록, ② 투표소 규모(투표인 수)가 작을수록, ③ 한 후보의 지지세가 압도적일수록** 산수적으로 더 자주 나타나는 자연 현상이다.

세 조건이 동일쌍을 실제로 키우는지, 한국·대만·프랑스의 실제 데이터로 §4~6에서 하나씩 확인하고, §9에서 종합합니다.

---

## 2. 무엇을, 얼마나 조사했나

의혹을 정밀하게 검증하기 위해, 한국의 여러 선거와 **대만과 프랑스 선거까지** 공식 데이터를 전수 수집했습니다.

| 데이터 | 단위·규모 | 입수 방법 |
|---|---|---|
| **2026 지방선거** (시도지사·국회의원) | 읍면동 단위 **7,489개**(사전 3,731 + 본투표 3,758) | 선관위 크롤링 |
| 2025 제21대 대선 (읍면동) | 읍면동 단위 7,108개 | 선관위 크롤링 |
| 역대 대선 5회 (제17~21대, 투표구) | 투표구(투표소) **69,443개** | 개방포털 전체파일 |
| 역대 총선 4회 (제19~22대, 투표구) | 투표구(투표소) **55,824개** | 개방포털 전체파일 |
| 역대 지방선거 4회 (제5~8회) 시도지사 | 읍면동 단위 **약 14,000개** | 개방포털 전체파일 |
| 대만 총통선거 2회 (2020·2024) | 투개표소 **35,021개** | 대만 선거DB 전체파일 |
| 프랑스 대선 2회 (2017·2022) | 투표소 **138,840개** | 프랑스 내무부 공개데이터 |

→ 합계 **32만 개가 넘는** 투표소·개표단위를 전수 분석했습니다(표본추출 없음). *(2025 대선은 §5-1용 읍면동·§5-2용 투표구 두 단위로 각각 분석.)*

- **"사전투표 투표소"의 정의** = 각 읍면동의 **관내사전투표** 득표(읍면동당 1개소).
- **비교용 대조군** = 같은 읍면동의 **선거일투표(본투표)**.
- ※ 한국의 **본투표 물리적 투표소(투표구)는 1만 개를 크게 웃돕니다**(과거 대선 기준 약 1.3~1.4만). 다만 읍면동당 1개인 사전투표소와 **1:1로 대응시켜 비교**하려고 본투표도 읍면동 단위(3,758)로 합산했습니다. 더 잘게(투표구) 나누면 동일쌍은 줄지 않고 오히려 **늘어납니다**(가설 ①).
...
- **② 몬테카를로 시뮬레이션 ('우연 기대' 계산법)** — 각 투표소의 **실제 투표 인원수는 그대로 두고**, 그 선거(시도·선거구)의 **평균 후보 지지율로 득표를 무작위 재배분**해 1·2위 동일쌍 수를 셉니다. 이를 **여러 번 반복한 평균**(분석에 따라 200~2,000번)이 "순전히 우연이라면 이만큼"인 **우연 기대값**입니다. 실제 투표소 크기를 그대로 쓰므로 "투표소마다 인원이 다르다"는 점이 자동 반영됩니다(작은 투표소가 많을수록 기대값도 커짐).
- **③ 실제 관측치와 비교**
...
**② 몬테카를로:** 가짜 선거 2,000번 결과 평균 **11.4쌍**(95% 5~19). 시뮬레이션은 **각 투표소의 실제 득표 규모를 그대로 고정**하므로 "동마다 유권자 수가 다르다"는 점을 정확히 반영합니다.
...
- **③ 무엇보다, 이 정도는 실제 선거에서 늘 나옵니다.** 분쟁 없던 2022 대선엔 전남 한 곳에서만 **15쌍**, 경북 5쌍이 나왔습니다(§5-2). 게다가 **같은 시뮬레이션이 2022 투표구에선 오히려 과대예측**(전남 기대 약 21 > 관측 15)할 만큼, 모형 기대값은 단위마다 들쑥날쑥해 절대 잣대가 못 됩니다. 모형 확률보다 **실제 선거 기록**이 더 믿을 만하고, 거기서 보면 광주·전남 4쌍은 정상 범위입니다.
...
### 5-2. "여당 텃밭에서만 나온다"? — 역대 대선으로 검증

흔한 반론: *"쏠림이 현 여당(민주당) 강세지역(호남)에서만 나오는 것 아니냐."* 그래서 **역대 대선 다섯 번**(제17~21대, 투표구 단위 약 6.9만 개) — 보수 승리(이명박·박근혜·윤석열)와 진보 승리(문재인·이재명)를 **모두** — 확인했습니다.
...
| 제21대 (이재명·진보) | 진보 | 155 | 226 |

> ※ 한국 대선 동일쌍은 모두 **투표구(물리적 투표소) 단위**입니다. 이 투표구별 데이터는 선관위 **개방포털**(data.nec.go.kr)의 사후 상세 아카이브에서 받았습니다 — 실시간 개표 시스템(info.nec.go.kr)은 동별 개표라 **읍면동까지만** 제공하므로, 역설적으로 개방포털 쪽이 더 잘게 나뉩니다. 2025 대선도 같은 방식으로 투표구로 통일했습니다(읍면동 기준인 §5-1과 **단위만 다른 같은 선거**). 단, **2026 지선은 선거 직후라 투표구별 아카이브가 아직 공개되지 않아** 부득이 읍면동 단위입니다(§8).

![과거 대선 전국 동일쌍](figures/fig_hist_national.png)

보수·진보가 번갈아 이긴 선거 **모두**에서 동일쌍은 수십~수백 쌍이고, **모두 우연 기대값 이하**(p=1.00)입니다. 권역별로 봐도:
...
### 5-3. 사전투표만 따로 보면 (사전투표 도입 2014년 이후 — 대선·총선·지선)

위는 본투표가 중심이었습니다. 사전투표가 있는 선거의 **사전투표(관내사전, 읍면동 단위)만** 떼어 봐도 결과는 같습니다.

| 선거 (사전투표·읍면동) | 사전투표소 | 1·2위 동일쌍(관측) | 우연 기대 |
|---|---|---|---|
| 대선 19대 (2017) | 3,491 | 14 | 70 |
| 대선 20대 (2022) | 3,510 | 2 | 14 |
| 대선 21대 (2025) | 3,554 | **0** | 20 |
| 총선 20대 (2016) | 3,466 | 32 | 53 |
| 총선 21대 (2020) | 3,485 | 2 | 5 |
| 총선 22대 (2024) | 3,551 | 4 | 3 |
| 지선 2026 (시도지사) | 3,558 | 9 | 10 |

- 사전투표소(읍면동) 단위라 수가 작습니다(0~32쌍). 대선·총선 **대부분 우연 기대값 이하**이고, 가장 많은 20대 총선(32)도 기대(53) 이하입니다. (22대 총선만 4 vs 3으로 사실상 동률 — 95% 구간 0~7 내.)
- (사전투표 도입 전인 17·18대 대선은 해당 없음. 21대 대선 본투표(6쌍)는 §5-1 참조.)

### 5-4. 총선에서도 똑같다 (역대 국회의원선거)

대선뿐 아니라 **국회의원선거(총선)**도 투표구 단위로 확인했습니다(개방포털 투표구별 파일). 우연 기대는 각 **선거구별 평균 지지율** 모형으로 계산했습니다.

| 총선 | 투표구 | 동일쌍(관측) | 우연 기대 |
|---|---|---|---|
| 제22대 (2024) | 14,259 | 257 | 311 (95% 274~345) |
| 제21대 (2020) | 14,330 | 208 | 273 (95% 244~305) |
| 제20대 (2016) | 13,765 | 203 | 290 (95% 258~325) |
| 제19대 (2012, **사전투표 도입 전**) | 13,470 | 130 | 159 (95% 134~184) |

- 총선도 동일쌍이 **수백 쌍씩, 모두 우연 기대값 이하**(p=1.00)입니다. **선거 종류(대선·총선·지선)를 가리지 않습니다.**
- **사전투표가 없던 19대 총선(2012)도 본투표에서 130쌍** 나옵니다 — 동일쌍은 **사전투표와 무관**하게 늘 존재합니다(17·18대 대선도 사전투표 이전 선거, §5-2). 사전투표 도입 전 선거에 동일쌍이 더 적은 것도 아닙니다.

### 5-5. 가장 직접적인 비교 — 역대 지방선거 시도지사 (2026과 같은 선거)

의혹의 2026 선거와 **같은 종류·같은 단위**(시도지사, 읍면동)인 역대 지방선거(5~8회)와 비교했습니다 — 가장 결정적인 대조입니다.

| 지선 시도지사 (읍면동) | 사전투표 동일쌍 (관측/기대) | 본투표 동일쌍 (관측/기대) |
|---|---|---|
| 제5회 (2010, 사전투표 전) | — (제도 없음) | 2 / 2 |
| 제6회 (2014) | 45 / 62 | 1 / 3 |
| 제7회 (2018) | 8 / 14 | 2 / 4 |
| 제8회 (2022) | 10 / 10 | 4 / 7 |
| **2026 (의혹의 선거)** | **9 / 10** | **4 / 5** |

> ※ 지방선거는 **본투표의 투표구별(투표소별) 개표결과가 공개돼 있지 않아** 모두 **읍면동(선거일투표·관내사전투표) 단위**로 분석했습니다 — 2026 지선과 동일 단위입니다(그래서 §5-2 대선·§5-4 총선의 투표구 기준보다 수가 작습니다). **5회(2010)는 사전투표 도입 전**이라 사전이 없고, '본투표'는 사전·선거일 구분이 없는 **전체 읍면동** 값입니다.

- **2026의 사전 9쌍·본투표 4쌍은 역대 지선과 똑같은 수준**입니다(사전 8~45·본 1~4). **비정상이 전혀 아닙니다.**
- 모든 회차가 우연 기대값 이하이거나 동률입니다.
- **6회(2014)가 사전 45로 가장 많은 건 첫 사전투표라 이용률이 낮아 읍면동당 사전 득표가 가장 작았기 때문**입니다(평균 **741표** vs 7·8회 1,607·1,830표). 작은 숫자일수록 겹치기 쉽기 때문(가설 ②)으로, 사전투표가 보편화돼 규모가 커지자 동일쌍은 오히려 **줄었습니다(45→8→10)**. 그래도 45는 우연 기대(62) **이하**입니다. — 사전투표율이 낮을수록 동일쌍이 많다는 게 조작과 정반대 방향임을 보여줍니다.
- **사전투표가 아예 없던 5회(2010)도 본투표에 2쌍** 나옵니다(사전 0 = 제도 도입 전, 데이터로도 확인).

> **질문② 답:** 동일쌍은 **2026년만의 현상도, 특정 진영의 현상도 아닙니다.** 진보·보수가 이긴 모든 선거에서, **대선·총선·지선 가리지 않고**, 사전·본투표 가리지 않고, 양 진영 텃밭 모두에서 나타나며, 늘 우연 기대값 이하입니다. **특히 같은 지방선거(6~8회 시도지사)와 견주면 2026은 완전히 평범합니다.**

---

## 6. 질문③ — 사전투표·전자개표 탓인가? 수기개표하면 깨끗한가? (대만·프랑스)
...
| 프랑스 2022 | **69,612** | **약 505** | **157,945** |

![투표소 단위 비교](figures/fig_tw_vs_kr.png)

- **① 투표소가 더 많다**(대만 1.8만, 프랑스 7만 vs 한국 1.4만) → 비교쌍이 훨씬 많음.
- **② 투표소당 투표인이 더 적다**(대만 784·프랑스 504 vs 한국 1,200~2,200표) → 1·2위가 작은 숫자라 겹치기 훨씬 쉬움(이쪽이 더 결정적).
- 한국 안에서도 투표소가 더 작은 19·20대가 18대보다 동일쌍이 몇 배 많습니다.

**프랑스는 더 극적입니다.** 음모론자들이 대만과 함께 "깨끗한 선거"로 거론하는 프랑스 역시 **종이투표·100% 수기개표·사전투표 없음**입니다. 그런데 투표소(bureau de vote)가 **약 7만 개**로, 한국의 선거일 투표소(투표구 약 1.4만 개)의 **5배 가까이** 많고 평균 504표로 작습니다. 프랑스 내무부 공식 투표소별 데이터로 확인했습니다.

| 프랑스 대선 1차 | 투표소 | 1·2위 동일쌍(관측) | 우연 기대 |
|---|---|---|---|
| 2022 (마크롱) | 69,612 | **157,945쌍** | 384,160 |
| 2017 (마크롱) | 69,228 | **202,504쌍** | 579,970 |

*(득표가 0인 빈 투표소는 제외한 수치입니다.)*

- 종이·수기개표 프랑스에 1·2위 동일쌍이 **16만~20만 쌍** — 한국(9쌍)·대만(2,395쌍)과 비교 불가하게 많습니다. **투표소가 가장 많고 가장 작기 때문**이지, 개표 방식이 깨끗해서가 아닙니다.
- 그리고 이 수십만 쌍조차 **우연 기대값(38만~58만)보다 적습니다**(p=1.00) — 다른 모든 선거와 똑같은 패턴입니다.

> **질문③ 답:** **사전투표를 없애고 종이·수기개표로 바꿔도 동일쌍은 줄기는커녕 오히려 폭증합니다**(대만 수천 쌍, 프랑스 수십만 쌍). 동일쌍은 개표 방식의 문제가 아니라 산수의 문제입니다.
...
선거도 똑같습니다. 투표소 3,731곳 → 비교쌍 **696만**, 득표수의 실질 경우의 수 **약 144만** → **696만÷144만 ≈ 5**. 동일쌍 몇 개는 산수적으로 **반드시** 나옵니다. 투표소가 많을수록, 투표소가 작을수록 더 많이 나옵니다 — 그래서 대만(수천 쌍)·프랑스(수십만 쌍)처럼 투표소가 많고 작은 나라에서 훨씬 많이 나오는 것입니다.
...
- **분석 단위:** 한국 본투표는 **동별 개표**라 선관위 공개 최소 단위가 **읍면동**입니다(2026·2025). 한국에서 **투표구(물리적 투표소, ~1.4만)** 단위로 받을 수 있는 건 **과거 대선**(개방포털 투표구별 파일)뿐이며, 이는 대만 투개표소(1.8만)·프랑스 bureau(7만)와 **같은 투표소 급**입니다. 그래서 **국가 간 비교(§6·§9)는 한국=과거 대선 투표구로 투표소 단위를 통일**했고, 읍면동 데이터(2026·2025)는 **읍면동끼리만** 비교했습니다(§5-1·§5-3). 단, 단위가 잘아질수록 동일쌍은 늘어나므로(가설 ①), 2026을 투표구로 쪼갰다면 동일쌍은 오히려 더 많아집니다 — 결론에 불리하게 작용하지 않습니다.
...
2. **2026년·특정 진영만의 현상이 아니다** — 역대 대선 5회·총선 4회·지선 4회(제5~8회, 같은 시도지사 선거), 양 진영 텃밭 모두에서 나타나며 늘 우연 이하. **특히 같은 지선과 비교하면 2026은 평범**. (5장)
3. **개표 방식 탓이 아니다** — 사전투표 없고 100% 수기개표하는 대만에 수천 쌍, 프랑스에 **수십만 쌍**. (6장)
4. **설령 조작이라 가정해도 동일쌍을 만들 이유가 없다** — 두 투표소 득표가 같든 다르든 총득표·당락은 동일하다. 표가 한 표도 늘지 않고, 일부러 맞추면 발각 위험만 커진다. 조작자라면 차라리 무작위로 만든다.

**그리고 서두의 가설은 데이터로 그대로 확인되었습니다.** 동일 득표쌍은 다음 세 조건이 강할수록 더 많이 나타납니다.

| 가설의 조건 | 데이터가 보여주는 것 |
|---|---|
| **① 투표소가 많을수록 ↑** | (선거일 기준) 한국 대선 55~289쌍(투표구 1.4만) < 대만 815~2,395쌍(1.8만) < **프랑스 16만~20만쌍(7만)** |
| **② 투표소가 작을수록 ↑** | 투표소당 평균 득표가 작을수록 급증(한국 1,200~2,200 > 대만 784 > 프랑스 504표); 한국 9쌍은 전부 '크기 쌍둥이' 작은 투표소 |
| **③ 한 후보가 압승일수록 ↑** | 호남(민주)·영남(보수)·전남(이재명 84%, 15쌍) 등 **압승 지역에 집중** — 압승하면 2위 이하 득표가 작아져 겹치기 쉬움 |

즉 동일 득표쌍은 **이 세 가지 산수적 조건의 자연스러운 산물**이지 "끼워 맞춘 흔적"이 아닙니다. 거꾸로, 조작 가설로는 이 규칙성(왜 하필 작은 투표소에, 압승 지역에, 투표소 많은 나라에 더 많은지)을 설명하지 못합니다.

| 의혹의 주장 | 검증 결과 |
|---|---|
| 동일 득표쌍이 존재한다 | ✔ 사실 |
| 그 개수가 비정상적으로 많다 | ✘ 거짓 — 우연 기대값 이하 |
| 사전투표/특정 진영/특정 개표방식 탓이다 | ✘ 거짓 — 본투표·타 선거·양 진영·수기개표 대만·프랑스 모두 동일 |
